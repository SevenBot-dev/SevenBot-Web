...
from flask import Flask, Blueprint, jsonify, make_response, redirect, render_template, request
...
def captcha():
    ...
    if not sessionid:
        return render_template("captcha/session.html", timeout=False)
    ...
    if not maincollecton.find_one({"sid": sessionid}):
        r = make_response(render_template("captcha/session.html", timeout=False))
        r.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
        return r
    elif time.time() - float(maincollecton.find_one({"sid": sessionid})["time"]) > 300:
        maincollecton.delete_one({"sid": sessionid})
        r = make_response(render_template("captcha/session.html", timeout=True))
        r.headers["Cache-Control"] = "no-cache, no-store, must-revalidate"
        return r
...
def check_ok():
    ...
    if r.json()["success"]:
        ...
        g = requests.get(
            f"https://discord.com/api/v9/guilds/{gid}", headers={"authorization": "Bot " + os.environ.get("token")}
        )
        gname = g.json()["name"]
        requests.post(
...
def make_session():
    ...
    if res.get("password") != os.environ.get("password"):
        return make_response(jsonify({"message": "You cannot access this endpoint."}), 403)

    shash = hashlib.md5(str(time.time()).encode()).hexdigest()[:16]
    maincollecton.insert_one(
        {"sid": shash, "uid": res["uid"], "gid": res["gid"], "rid": res["rid"], "time": time.time()}
    )
