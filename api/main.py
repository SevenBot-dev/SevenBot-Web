...
def can_access(password, gid):
    ...
    guilds = request_after("get", "discord.com/api/users/@me/guilds", headers={"password": "Bearer " + password}).json()
...
def afk_post():
    ...
    if t is not None:
        ...
        auth = auth_requests.OAuth1Auth(
            os.environ.get("twitter_consumer_key"), os.environ.get("twitter_consumer_secret"), t["token"], t["secret"]
        )
...
def dbl():
    ...
    requests.post(
        ...
        json={"content": f"<@!{request.json['user']}> さんが高評価をしてくれました！", "allowed_mentions": {"parse": []}},
    )
