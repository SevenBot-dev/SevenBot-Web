...
def check_setting_update(guild_id: int):
    ...
    user_info = current_app.dash_user_caches.get(request.headers["authorization"])
...
def set_user_cache(token_data):
    ...
    current_app.dash_user_caches[token_hash] = data
    return data
...
def load_logged_in_user():
    ...
    if not (token := request.cookies.get("token")):
        ...
        return
    if not current_app.dash_user_caches.get(token):
    ...
    if token is not None:
        token_data = current_app.dash_user_caches.get(token)
        if token_data is None:
...
def manage(guild_id):
    if not (user_info := current_app.dash_user_caches.get(request.cookies.get("token"))):
        return redirect("/?popup=ログインしていません。")
